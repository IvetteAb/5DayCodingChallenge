...
Take a look at lines 22 and 33 . You'll see that there are two of what are called event listeners added as attributes to the ingredients and preparation sections . 
Their job is to listen for and react to two particular events . One reacts when a user's mouse passes into the section area ( onmouseover ) and the second reacts when 
a user's mouse passes out of the section ( onmouseout ).

When one of these events occurs the appropriate event listener calls a function . For example, when the user's mouse passes into the ingredients section the 
onmouseover event listener will call a function named ingredientsHover() . This is function is written by the programmer.
...
Create the function that is called when the ingredients onmouseover event is triggered. The code is shown below. 
Notice how the function name tries to indicate its purpose */
function ingredientsHover(){
 
}

/* Add the code that changes the ingredients icon size. This will be placed inside the function's curly braces {}. The code is shown below. 
Try and understand how it works. Read it from left to right. The document represents the web page */
...
/*Add the code that resets the ingredients icon size to its default size. This will be placed inside the function's curly braces {}. The code is shown below. 
...
/* Add the code that changes the preparation icon size. This will be placed inside the function's curly braces {}. The code is shown below. 
Try to understand how it works. */
...
/* Add the code that resets the preparation icon size to its default size. This will be placed inside the function's curly braces {}. The code is shown below. 
Try to understand how it works. */
...
- all HTML elements are seen as objects so we start out we start off with the document (i.e. the webpage) object, which is the the parent of everything and then we 
	say get the element by ID
- Q: well which ID? while ingredients so find me an element (<h2>)that has an ID of ingredients, then find its first element child ()<i>) and then find that's first
	elements child and once you're there I want you to modify the style and in particular the font size & set that to 300% 
- same for retuning the size back to normal (100%)
*/
